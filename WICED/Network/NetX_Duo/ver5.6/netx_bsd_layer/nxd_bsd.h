...
/* Define the extended notification services for NetX BSD sockets. This requires
   NetX v5.4 or later, or NetX Duo v5.6 or later.  A patch is available for NetX 5.3.  
   Services include asynchronous connect, accept, and disconnect notification.  Socket
   options include timed wait, linger, and error handling. See the BSD readme documentation
   for more details.  
#ifndef NX_EXTENDED_BSD_SOCKET_SUPPORT         
...
#define NX_BSD_TIMED_WAIT_TIMEOUT           (60 * NX_CPU_TICKS_PER_SECOND)   /* Timeout for sockets in the Timed Wait state (socket must     */
                                                                             ...
                                                                             /* Also requires NX_EXTENDED_BSD_SOCKET_SUPPORT be enabled.     */

#ifndef NX_BSD_TW_TIMER_RATE                 
#define NX_BSD_TW_TIMER_RATE                (5 * NX_CPU_TICKS_PER_SECOND)   /* Rate at which Timed Wait timer checks sockets in Timed Wait state*/
#endif                                                                      /* No effect if NX_EXTENDED_BSD_SOCKET_SUPPORT not defined or if  */
                                                                            /* if REUSEADDR enabled.                                          */

#ifndef NX_BSD_LINGER_TIMER_RATE                 
#define NX_BSD_LINGER_TIMER_RATE            (1 * NX_CPU_TICKS_PER_SECOND)   /* Rate at which socket Linger timer checks sockets in Linger state*/
#endif                                                                      /* No effect if NX_EXTENDED_BSD_SOCKET_SUPPORT not defined.        */                                          
...
/* If extended features are enabled, this option will compile BSD with the logic and ThreadX resources 
   for supporting the socket options Linger and Timed Wait socket options. The default is disabled. */ 
#define NX_EXTENDED_BSD_LINGER_AND_TIMED_WAIT 
...
#ifdef  NX_EXTENDED_BSD_LINGER_AND_TIMED_WAIT   
...
#define SO_NONBLOCK                         0x4000                  /* Option to enable non blocking on a file (e.g. socket)                */
...
#define SO_SNDBUFFORCE  32  /* Enable setting trasnmit buffer size overriding user limit (admin privelege) */
#define SO_RCVBUFFORCE  33  /* Enable setting trasnmit buffer size overriding user limit (admin privelege) */
...
struct sock_linger
{
    INT linger_onoff;                       /* 0 = disabled; 1 = enabled; default = 0;*/
    INT linger_time;                        /* linger time in seconds; default = 0;*/
};
...
/* Set parameters for the BSD thread task to wait for and handle asynchronous events. */
#define     NX_BSD_TASK_MEMORY      1024
#define     NX_BSD_THREAD_PRIORITY  1
...
typedef struct NX_BSD_SOCKET_STRUCT
{
    ...
    NXD_ADDRESS         nx_bsd_socket_udp_default_host_ip;
    UINT                nx_bsd_socket_udp_default_host_port;
#ifdef NX_EXTENDED_BSD_SOCKET_SUPPORT 
...
INT  bsd_initialize (NX_IP *default_ip, NX_PACKET_POOL *default_pool, CHAR *bsd_memory_not_used);
...
UINT inet_addr(const CHAR *buffer, ULONG *address);
INT  bind(INT sockID, struct sockaddr *localAddress, INT addressLength);
INT  connect(INT sockID, struct sockaddr *remoteAddress, INT addressLength);
VOID nx_bsd_socket_tcp_disconnect_handler(NX_TCP_SOCKET *socket_ptr);
...
VOID nx_bsd_socket_tcp_disconnect_extended_handler(NX_TCP_SOCKET *socket_ptr);
UINT nx_bsd_set_socket_disconnect_complete_notify(VOID (*user_bsd_disconnect_complete_cb)(NX_TCP_SOCKET *socket_ptr));
UINT nx_bsd_set_socket_establish_notify(VOID (*user_establish_cb)(NX_TCP_SOCKET *socket_ptr));
INT  setsockopt(INT sockID, INT option_level, INT option_name, const void *option_value, INT option_length);
INT  soc_close_extended( INT sockID);
#endif /* NX_EXTENDED_BSD_SOCKET_SUPPORT */
