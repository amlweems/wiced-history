...
#define NX_DHCP_PACKET_PAYLOAD          (NX_BOOT_BUFFER_SIZE + NX_PHYSICAL_HEADER +  sizeof(NX_IPV4_HEADER) + sizeof(NX_UDP_HEADER))
...
#define NX_PACKET_ALLOCATE_TIMEOUT      (NX_DHCP_TIME_INTERVAL * _nx_system_ticks_per_second)
#endif
...
typedef struct NX_DHCP_STRUCT 
{
    ...
    ULONG           nx_dhcp_xid;                /* Transaction ID for all transmissions and to validate     */
                                                /*   responses                                              */
    ULONG           nx_dhcp_ip_address;         /* The IP Address assigned by the server (or the last IP    */
                                                /*   address)                                               */
...
UINT        _nx_dhcp_get_response(NX_DHCP *dhcp_ptr, ULONG wait_option, NX_PACKET **packet_ptr);
...
UINT        _nx_dhcp_client_send_arp_probe(NX_DHCP *dhcp_ptr, ULONG ip_address_to_probe, UINT timeout);
