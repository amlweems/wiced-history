    ...
    if ( ( IP_HANDLE(interface)->nx_ip_driver_link_up == NX_TRUE ) && ( wiced_wifi_is_ready_to_transceive( interface ) == WICED_SUCCESS ) )
    {
        switch ( driver->nx_ip_driver_command )
        {
            case NX_LINK_PACKET_SEND:
                ...
                {
                    nx_wiced_add_ethernet_header( IP_HANDLE(interface), packet_ptr, driver->nx_ip_driver_physical_address_msw, driver->nx_ip_driver_physical_address_lsw, (USHORT) WICED_ETHERTYPE_IPv6 );
                }
                else
                {
            ...
            case NX_LINK_PACKET_BROADCAST:
                ...
                {
                    nx_wiced_add_ethernet_header( IP_HANDLE(interface), packet_ptr, (ULONG) 0xFFFF, (ULONG) 0xFFFFFFFF, (USHORT) WICED_ETHERTYPE_IPv6 );
                }
                else
                {
                    wiced_assert("Bad packet IP version", 0 != 0);
