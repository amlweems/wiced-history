...
/*                                            multihome support, added    */ 
/*                                            contants for minimum packet */  
/*                                            payload, and adjusted the   */ 
/*                                            DHCP offset end constant,   */ 
/*                                            resulting in version 5.2    */ 
/*                                                                        */ 
/**************************************************************************/ 

#ifndef  NX_DHCP_SERVER_H
#define  NX_DHCP_SERVER_H
...
/* The DHCP client needs to support multihoming if
   the current NetX is NetX v5.3 or later. */
...
#define NX_DHCP_PACKET_ALLOCATE_TIMEOUT         200   /* Approximately 20 ms ticks  */ 
#endif
...
/* Define the length of the client's unique identifier.  Must be at 
   least large enough to hold the hardware mac address and hardware type 
   (7 bytes)
*/

#ifndef NX_DHCP_CLIENT_IDENTIFIER_MAX
#define NX_DHCP_CLIENT_IDENTIFIER_MAX           7
#endif
...
#define NX_DHCP_OFFSET_END             NX_BOOT_BUFFER_SIZE     
                                              /* End of BOOTP buffer (NX_DHCP_PACKET_PAYLOAD-(IP, UDP and  */
                                              /* 4 byte aligned Ethernet headers)                          */
...
#define NX_DHCP_OPTION_ROUTER_SIZE      NX_DHCP_OPTION_ADDRESS_SIZE
...
#define NX_DHCP_OPTION_DHCP_SERVER_ID   54
...
#define NX_DHCP_OPTION_CLIENT_ID_SIZE   7  
...
#define NX_DHCP_ALREADY_STARTED                 0x90    /* DHCP was already started      */
#define NX_DHCP_NOT_STARTED                     0x91    /* DHCP was not started when stop was issued   */
...
#define NX_DHCP_BAD_INTERFACE_INDEX             0xA1    /* Interface index exceeds the NX_MAX_PHYSICAL_INTERFACES limit.  */
...
typedef struct NX_DHCP_CLIENT_STRUCT 
{
    ...
    UCHAR           nx_dhcp_client_identifier[NX_DHCP_CLIENT_IDENTIFIER_MAX];
                                                 /* Optional unique identier - typically the client HW addr and HW type. */
...
typedef struct NX_DHCP_INTERFACE_IP_ADDRESS_STRUCT
{
    ...
    UCHAR           owner[NX_DHCP_CLIENT_IDENTIFIER_MAX];
                                                 /* Who the address is assigned to; only known if this server assigned it. */
...
#ifdef MULTI_HOME_NETX                                     /* IP address available to assign to DHCP Client. */
...
UINT        nx_dhcp_create_server_ip_address_list_text(NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, CHAR *input_address_list, UINT *addresses_added);
...
UINT        _nx_dhcp_create_server_ip_address_list_text(NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, CHAR *input_address_list, UINT *addresses_added);
UINT        _nxe_dhcp_create_server_ip_address_list_text(NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, CHAR *input_address_list, UINT *addresses_added);
...
/* Internal functions. */
VOID        _nx_dhcp_server_thread_entry(ULONG ip_instance);
VOID        _nx_dhcp_slow_periodic_timer_entry(ULONG info);
VOID        _nx_dhcp_fast_periodic_timer_entry(ULONG info);
UINT        _nx_dhcp_listen_for_messages(NX_DHCP_SERVER *dhcp_ptr);
UINT        _nx_dhcp_respond_to_client_message(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
//UINT        _nx_dhcp_extract_information(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT **dhcp_client_ptr, NX_PACKET *packet_ptr, UINT iface_index);
UINT        _nx_dhcp_process_option_data(NX_DHCP_CLIENT *dhcp_ptr, CHAR *buffer, ULONG value, UINT get_option_data, UINT size);
UINT        _nx_dhcp_add_option(UCHAR *bootp_message, UINT option, UINT size, ULONG value);
UINT        _nx_dhcp_add_requested_option(NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, UCHAR *buffer, UINT option);
UINT        _nx_dhcp_add_option_string(UCHAR *bootp_message, UINT option, UINT size, UCHAR *value);
UINT        _nx_dhcp_set_server_options(NX_DHCP_SERVER *dhcp_ptr, CHAR *buffer, UINT buffer_length);
UINT        _nx_dhcp_load_server_options(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr, UCHAR *buffer, UINT option_type);
UINT        _nx_dhcp_parse_next_option(CHAR **buffer, UINT *digit, UINT length);
//UINT        _nx_dhcp_get_data(UCHAR *data, UINT size, ULONG *value);
//VOID        _nx_dhcp_store_data(UCHAR *data, UINT size, ULONG value);
UINT        _nx_dhcp_clear_client_session(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
UINT        _nx_dhcp_validate_client_message(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
UINT        _nx_dhcp_find_client_record_by_chaddr(NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, ULONG client_mac_msw, ULONG client_mac_lsw,NX_DHCP_CLIENT **dhcp_client_ptr, UINT add_on);
UINT        _nx_dhcp_find_client_record_by_ip_address(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT **dhcp_client_ptr, UINT iface_index, ULONG assigned_ip_address);
UINT        _nx_dhcp_server_assign_ip_address(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr);
UINT        _nx_dhcp_find_interface_table_ip_address(NX_DHCP_SERVER *dhcp_ptr, UINT iface_index, ULONG ip_address, NX_DHCP_INTERFACE_IP_ADDRESS **return_interface_address);
UINT        _nx_dhcp_update_assignable_ip_address(NX_DHCP_SERVER *dhcp_ptr, NX_DHCP_CLIENT *dhcp_client_ptr, ULONG ip_address, UINT assign_status); 
UINT        _nx_dhcp_find_ip_address_owner(UCHAR *iface_owner, NX_DHCP_CLIENT *client_record_ptr, UINT *assigned_to_client);
UINT        _nx_dhcp_record_ip_address_owner(UCHAR *iface_owner, NX_DHCP_CLIENT *client_record_ptr);
