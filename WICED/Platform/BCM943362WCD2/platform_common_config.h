/*
 * Copyright 2013, Broadcom Corporation
 * All Rights Reserved.
 *
 * This is UNPUBLISHED PROPRIETARY SOURCE CODE of Broadcom Corporation;
 * the contents of this file may not be disclosed to third parties, copied
 * or duplicated in any form, in whole or in part, without the prior
 * written permission of Broadcom Corporation.
 */
#pragma once

#include "platform.h"
#include "platform_sleep.h"

/******************************************************
 *                      Macros
 ******************************************************/

/******************************************************
 *                    Constants
 ******************************************************/

/* The CPU Clock Frequency (CPU_CLOCK_HZ) is independently defined in <WICED-SDK>/Wiced/Platform/BCM943362WCD2/BCM943362WCD2.mk
 */
#define HSE_SOURCE              RCC_HSE_ON               /* Use external crystal                 */
#define AHB_CLOCK_DIVIDER       RCC_SYSCLK_Div1          /* AHB clock = System clock             */
#define APB1_CLOCK_DIVIDER      RCC_HCLK_Div2            /* APB1 clock = AHB clock / 2           */
#define APB2_CLOCK_DIVIDER      RCC_HCLK_Div2            /* APB2 clock = AHB clock / 2           */
#define PLL_SOURCE              RCC_PLLSource_HSE_Div2   /* PLL source = external crystal / 2    */
#define PLL_MULTIPLIER_CONSTANT RCC_PLLMul_9             /* PLL Multiplier = 9                   */
#define SYSTEM_CLOCK_SOURCE     RCC_SYSCLKSource_PLLCLK  /* System clock source = PLL clock      */
#define SYSTICK_CLOCK_SOURCE    SysTick_CLKSource_HCLK   /* SysTick clock source = AHB clock     */
#define INT_FLASH_WAIT_STATE    FLASH_Latency_2          /* Internal flash wait state = 2 cycles */

#define SPI_BUS_CLOCK_BANK      GPIOB
#define SPI_BUS_MISO_BANK       GPIOB
#define SPI_BUS_MOSI_BANK       GPIOB
#define SPI_BUS_CS_BANK         GPIOC
#define SPI_IRQ_BANK            GPIOC
#define SPI_BUS_CLOCK_BANK_CLK  RCC_APB2Periph_GPIOB
#define SPI_BUS_MISO_BANK_CLK   RCC_APB2Periph_GPIOB
#define SPI_BUS_MOSI_BANK_CLK   RCC_APB2Periph_GPIOB
#define SPI_BUS_CS_BANK_CLK     RCC_APB2Periph_GPIOC
#define SPI_IRQ_CLK             RCC_APB2Periph_GPIOC
#define SPI_BUS_CLOCK_PIN       13
#define SPI_BUS_MISO_PIN        14
#define SPI_BUS_MOSI_PIN        15
#define SPI_BUS_CS_PIN          11
#define SPI_IRQ_PIN             9

#define SDIO_OOB_IRQ_BANK       GPIOB
#define SDIO_CLK_BANK           GPIOC
#define SDIO_CMD_BANK           GPIOD
#define SDIO_D0_BANK            GPIOC
#define SDIO_D1_BANK            GPIOC
#define SDIO_D2_BANK            GPIOC
#define SDIO_D3_BANK            GPIOC
#define SDIO_OOB_IRQ_BANK_CLK   RCC_APB2Periph_GPIOB
#define SDIO_CLK_BANK_CLK       RCC_APB2Periph_GPIOC
#define SDIO_CMD_BANK_CLK       RCC_APB2Periph_GPIOD
#define SDIO_D0_BANK_CLK        RCC_APB2Periph_GPIOC
#define SDIO_D1_BANK_CLK        RCC_APB2Periph_GPIOC
#define SDIO_D2_BANK_CLK        RCC_APB2Periph_GPIOC
#define SDIO_D3_BANK_CLK        RCC_APB2Periph_GPIOC
#define SDIO_OOB_IRQ_PIN        0
#define SDIO_CLK_PIN            12
#define SDIO_CMD_PIN            2
#define SDIO_D0_PIN             8
#define SDIO_D1_PIN             9
#define SDIO_D2_PIN             10
#define SDIO_D3_PIN             11

#define WL_GPIO0_BANK           GPIOB
#define WL_GPIO0_PIN            0
#define WL_GPIO0_BANK_CLK       RCC_APB2Periph_GPIOB
#define WL_GPIO1_BANK           GPIOB
#define WL_GPIO1_PIN            1
#define WL_GPIO1_BANK_CLK       RCC_APB2Periph_GPIOB

#define WL_REG_ON_BANK          GPIOB
#define WL_REG_ON_PIN           GPIO_Pin_2
#define WL_REG_ON_BANK_CLK      RCC_APB2Periph_GPIOB

#define WL_RESET_BANK           GPIOB
#define WL_RESET_PIN            GPIO_Pin_5
#define WL_RESET_BANK_CLK       RCC_APB2Periph_GPIOB

#define STDIO_USART                    USART1
#define STDIO_TX_PIN                   GPIO_Pin_9
#define STDIO_RX_PIN                   GPIO_Pin_10
#define STDIO_TX_PINSOURCE             GPIO_PinSource9
#define STDIO_RX_PINSOURCE             GPIO_PinSource10
#define STDIO_TX_BANK                  GPIOA
#define STDIO_RX_BANK                  GPIOA
#define STDIO_TX_BANK_CLK              RCC_APB2Periph_GPIOA
#define STDIO_RX_BANK_CLK              RCC_APB2Periph_GPIOA
#define STDIO_GPIO_ALTERNATE_FUNCTION  GPIO_AF_USART1
#define STDIO_TX_GPIO_PERIPH_CLOCK     RCC_APB2Periph_GPIOA
#define STDIO_RX_GPIO_PERIPH_CLOCK     RCC_APB2Periph_GPIOA
#define STDIO_IRQ_CHANNEL              37
#define STDIO_PERIPH_CLOCK             RCC_APB2Periph_USART1
#define STDIO_CLOCK_CMD                RCC_APB2PeriphClockCmd
#define STDIO_TX_CLK_CMD               RCC_APB2PeriphClockCmd
#define STDIO_RX_CLK_CMD               RCC_APB2PeriphClockCmd

//#define USART3_TX_BANK          GPIOB
//#define USART3_TX_PERIPH_CLK    RCC_APB2Periph_GPIOB
//#define USART3_TX_PIN           GPIO_Pin_10
//#define USART3_RX_BANK          GPIOB
//#define USART3_RX_PERIPH_CLK    RCC_APB2Periph_GPIOB
//#define USART3_RX_PIN           GPIO_Pin_11
//#define USART3_IRQ_CHANNEL      39
//#define USART3_PERIPH_CLK       RCC_APB1Periph_USART3
//#define USART3_CLK_CMD          RCC_APB1PeriphClockCmd

/*  Bootloader LED D1 */
#define BOOTLOADER_LED_GPIO     WICED_LED1
#define BOOTLOADER_LED_ON_STATE WICED_ACTIVE_HIGH

 /* Bootloader Button S1 */
#define BOOTLOADER_BUTTON_GPIO          WICED_BUTTON1
#define BOOTLOADER_BUTTON_PRESSED_STATE WICED_ACTIVE_LOW

/* The number of UART interfaces this hardware platform has */
#define NUMBER_OF_UART_INTERFACES  2

#define STDIO_UART       WICED_UART_1

/******************************************************
 *                   Enumerations
 ******************************************************/

/******************************************************
 *                 Type Definitions
 ******************************************************/

/******************************************************
 *                    Structures
 ******************************************************/

/******************************************************
 *                 Global Variables
 ******************************************************/

/******************************************************
 *               Function Declarations
 ******************************************************/
