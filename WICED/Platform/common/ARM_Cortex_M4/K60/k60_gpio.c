...
typedef struct
{
    k60_gpio_irq_callback callback;
    void*                 arg;
}k60_gpio_irq_data_t;
...
typedef struct
{
    ...
    k60_gpio_irq_data_t data[GPIO_LINES_PER_PORT]; // Callback and arg for all lines
}k60_gpio_port_irq_data_t;
...
static k60_gpio_port_irq_data_t gpio_irq_data[GPIO_PORTS];
...
#if 0 /* not used */
wiced_result_t k60_gpio_management_init( void )
{
