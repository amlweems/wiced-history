...
static host_semaphore_type_t dma_finished_semaphore;
...
static wiced_result_t     sdio_transfer_data         ( bus_transfer_direction_t direction, sdio_block_size_t block_size, uint32_t* data, uint16_t data_size );
void dma_irq( void );
...
void sdio_irq( void )
{
    ...
    if ( ( SDIO->STA & ( SDIO_STA_CCRCFAIL | SDIO_STA_DCRCFAIL | SDIO_STA_TXUNDERR | SDIO_STA_RXOVERR  | SDIO_STA_STBITERR )) != 0 )
    {
...
void dma_irq( void )
{
    ...
    DMA2->LIFCR = (uint32_t) (0x3F << 22); /* Clear interrupt */
    result = host_rtos_set_semaphore( &dma_finished_semaphore, WICED_TRUE );
    wiced_assert( "failed to set dma semaphore", result == WICED_SUCCESS );  /* check result if in debug mode */
    /*@-noeffect@*/
...
wiced_result_t host_platform_bus_init( void )
{
    ...
    if ( WICED_SUCCESS != ( result = host_rtos_init_semaphore( &dma_finished_semaphore ) ) )
    {
        return result;
    }
    ...
    nvic_init_structure.NVIC_IRQChannelPreemptionPriority = (uint8_t) 0x6; /* Must be lower priority than the value of configMAX_SYSCALL_INTERRUPT_PRIORITY otherwise FreeRTOS will not be able to mask the interrupt */
    ...
    SDIO_DeInit( );
    sdio_init_structure.SDIO_ClockDiv       = (uint8_t) 120; /* 0x78; clock is taken from APB high speed bus... About 400KHz */
...
//    GPIO_InitTypeDef gpio_init_structure;
    ...
    result = host_rtos_deinit_semaphore( &dma_finished_semaphore );
...
restart:
    ...
    if ( command == SDIO_CMD_53 )
    {
        ...
        loop_count = (uint32_t) COMMAND_FINISHED_CMD53_TIMEOUT_LOOPS;
        while ( ( SDIO->STA & SDIO_STA_CMDREND ) == 0 )
        {
            loop_count--;
        ...
        {
//            wiced_assert( "SDIO communication failure - 'General' error signaled in SDIO response to command 53", 0 );
            goto restart;
        }

        result = sdio_transfer_data( direction, block_size, data, data_size );
...
static wiced_result_t sdio_transfer_data( bus_transfer_direction_t direction, sdio_block_size_t block_size, uint32_t* data, uint16_t data_size )
{
    ...
    result = host_rtos_get_semaphore( &dma_finished_semaphore, (uint32_t) 50, WICED_TRUE );
    if ( result != WICED_SUCCESS )
    {
        return result;
    }
