             ...
             ( memcmp( &message->fixed.oui, P2P_OUI, 3 ) == 0 ) )
        {
            ...
            switch (message->fixed.p2p_sub_type)
            {
                case P2P_GO_NEGOTIATION_REQUEST:
                    if ( ( workspace->p2p_current_state != P2P_STATE_GROUP_OWNER ) && ( workspace->p2p_current_state != P2P_STATE_GROUP_CLIENT ) )
                    {
                        ...
                        else
                        {
                            ...
                            workspace->group_candidate.operating_channel.channel = p2p_tlv->data[4];
                            ...
                            if ( p2p_tlv != NULL )
                            {
                                memcpy( &workspace->group_candidate.p2p_capability, p2p_tlv->data, 2);
                            }
                        }
                ...
                case P2P_GO_NEGOTIATION_RESPONSE: /*  GO Response */
                    ...
                    if (p2p_tlv != NULL)
                    {
                        memcpy(&device->channel_list.country_string, &p2p_tlv->data[0], 3);
                        device->channel_list.operating_class = p2p_tlv->data[3];
                        device->channel_list.number_of_channels = p2p_tlv->data[4];
                        memcpy(&device->channel_list.channel_list, &p2p_tlv->data[5], device->channel_list.number_of_channels);

                        /* If we're the group owner we have to select an acceptable operating channel. The following does not yet account for checking of operating class etc */
                        if (workspace->i_am_group_owner == 1)
                        {
                            int i, j;
                            wiced_bool_t channel_selected = WICED_FALSE;

                            for (i = 0; i < device->channel_list.number_of_channels; i++)
                            {
                                if ( workspace->operating_channel.channel == device->channel_list.channel_list[i] )
                                {
                                    channel_selected = WICED_TRUE;
                                    workspace->group_candidate.operating_channel.channel = device->channel_list.channel_list[i];
                                    break;
                                }
                            }

                            i = 0;
                            while ( ( channel_selected == WICED_FALSE ) && ( i < device->channel_list.number_of_channels ) )
                            {
                                for (j = 0; j < workspace->channel_list.number_of_channels; j++)
                                {
                                    if (device->channel_list.channel_list[i] == workspace->channel_list.channel_list[j])
                                    {
                                        workspace->group_candidate.operating_channel.channel = workspace->channel_list.channel_list[j];
                                        channel_selected = WICED_TRUE;
                                        break;
                                    }
                                }
...
besl_result_t p2p_process_probe_request( p2p_workspace_t* workspace, const uint8_t* data, uint32_t data_length )
{
    ...
    besl_host_get_time( &device->last_seen_time );
    if ( device->status != P2P_DEVICE_INVALID )
    {
        goto exit;
    }
