...
besl_result_t besl_supplicant_start( supplicant_workspace_t* workspace )
{
    ...
    result = (besl_result_t) host_rtos_create_thread_with_arg( &host_workspace->thread, wiced_supplicant_thread, "supplicant", host_workspace->thread_stack, SUPPLICANT_THREAD_STACK_SIZE, RTOS_HIGHER_PRIORTIY_THAN(RTOS_DEFAULT_THREAD_PRIORITY), (uint32_t) workspace );
...
besl_result_t besl_supplicant_deinit( supplicant_workspace_t* workspace )
{
    ...
    if ( host_workspace != NULL )
    {
        IF_TO_WORKSPACE( host_workspace->interface ) = NULL;
