...
/*                                            version 5.6                 */
/*  01-31-2013     Yuxin Zhou               Modified comment(s), moved    */
...
/*                                            added APIs to retrieve IPv4 */
/*                                            and IPv6 router entries,    */
/*                                            added index number into the */ 
/*                                            IPv6 address structure and  */ 
/*                                            the IP interface structure, */ 
/*                                            remove function declaration */ 
/*                                            nx_icmp_interface_ping6,    */ 
/*                                            added support for queue     */
/*                                            depth notify callback,      */
/*                                            resulting in version 5.7    */
/*  xx-xx-xxxx     Yuxin Zhou               Modified comment(s), and      */ 
/*                                            added new APIs for IPv4     */
/*                                            multicast, added definition */
/*                                            for reassemly time, added   */
/*                                            new service                 */
...
#define __NETXDUO_MINOR_VERSION__ 7
...
/* For backward compatibility, convert map NX_RAW_PACKET_FILTER_ENABLE to NX_ENABLE_IP_RAW_PACKET_FILTER */
...
#if defined(FEATURE_NX_IPV6) || defined(NX_IPSEC_ENABLE)
    UCHAR       nx_packet_option_state;  /* Used by IPv6, indicating the current option
                                            being processed. */
#endif /* FEATURE_NX_IPV6 or NX_IPSEC_ENABLE */

#ifdef FEATURE_NX_IPV6
    UCHAR       nx_packet_destination_header;
    USHORT      nx_packet_option_offset;
#elif defined(NX_IPSEC_ENABLE)
    /* 4-byte align. */
    UCHAR       nx_packet_reserved[3];
#endif /* FEATURE_NX_IPV6 */    
...
#define nx_ipv4_multicast_interface_join(p,g,i) _nx_igmp_multicast_interface_join_internal(p,g,i,NX_WAIT_FOREVER)
#define nx_ipv4_multicast_leave                 _nx_igmp_multicast_leave_internal
...
#define nx_ipv4_multicast_interface_join(p,g,i) _nxe_igmp_multicast_interface_join_internal(p,g,i,NX_WAIT_FOREVER)
#define nx_ipv4_multicast_leave                 _nxe_igmp_multicast_leave_internal
...
UINT    nx_ip_interface_info_get(NX_IP *ip_ptr, UINT interface_index, const CHAR **interface_name, ULONG *ip_address,
                              ULONG *network_mask, ULONG *mtu_size, ULONG *physical_address_msw, ULONG *physical_address_lsw);
UINT    nx_ip_interface_address_mapping_configure(NX_IP *ip_ptr, UINT interface_index, UINT mapping_needed);
UINT    nx_ip_interface_physical_address_get(NX_IP *ip_ptr, UINT interface_index, ULONG *physical_msw, ULONG *physical_lsw);
...
UINT    nx_ip_interface_physical_address_set(NX_IP *ip_ptr, UINT interface_index, ULONG physical_msw, ULONG physical_lsw, UINT update_driver);
...
#ifndef NX_DISABLE_ERROR_CHECKING
UINT    _nxe_ip_raw_packet_source_send(NX_IP *ip_ptr, NX_PACKET **packet_ptr_ptr, ULONG destination_ip, UINT address_index, ULONG type_of_service);
...
UINT    nxd_ipv6_default_router_get(NX_IP *ip_ptr, UINT interface_index, NXD_ADDRESS *router_addr, ULONG *router_lifetime, ULONG *prefix_length);
UINT    nxd_ipv6_default_router_number_of_entries_get(NX_IP *ip_ptr, UINT interface_index, UINT *num_entries);
UINT    nxd_ipv6_default_router_entry_get(NX_IP *ip_ptr, UINT interface_index, UINT entry_index, NXD_ADDRESS *router_addr, ULONG *router_lifetime, ULONG *prefix_length, ULONG *configuration_method);
...
#ifdef NX_TCP_QUEUE_DEPTH_UPDATE_NOTIFY_ENABLE     
UINT    nx_tcp_socket_queue_depth_notify_set(NX_TCP_SOCKET *socket_ptr,  VOID (*tcp_socket_queue_depth_notify)(NX_TCP_SOCKET *socket_ptr));
#endif
...
UINT    _nxe_udp_socket_source_send(NX_UDP_SOCKET *socket_ptr, NX_PACKET **packet_ptr, ULONG ip_address, UINT port, UINT address_index);
#else
UINT    _nx_udp_socket_send(NX_UDP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, ULONG ip_address, UINT port);
UINT    _nx_udp_socket_source_send(NX_UDP_SOCKET *socket_ptr, NX_PACKET *packet_ptr, ULONG ip_address, UINT port, UINT address_index);
...
UINT    nxd_ip_raw_packet_source_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, NXD_ADDRESS *destination_ip, UINT address_index, ULONG protocol, UINT ttl, ULONG tos);
