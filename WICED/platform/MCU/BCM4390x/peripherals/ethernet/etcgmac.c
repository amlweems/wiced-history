...
/* WICED: In case of 43909 it does not make sense to apply a global default
 * dma rx header room to all interfaces, every interface should have their
 * own rx header room. Currently there is no extra room needed for ethernet as forwarding
 * feature is not implemented.
 */
#define ETC_DMARXHDROOM 0
...
static void chipphyloopback(ch_t *ch, uint32 mode);
#ifdef BCMDBG
...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	BCM_REFERENCE(boardflags);
	BCM_REFERENCE(boardtype);
	...
	if (!et_set_addrs(etc)) { /* WICED: move address configuration to WICED part of driver */
		ET_ERROR(("et%d: chipattach: address configuratiin failed\n", etc->unit));
		goto fail;
	}
	...
	ch->di[0] = dma_attach(osh, name, ch->sih,
	                       ...
	                       NTXD, NRXD, RXBUFSZ, ETC_DMARXHDROOM, NRXBUFPOST, HWRXOFF,
	                       &et_msg_level);
	...
	ch->di[1] = dma_attach(osh, name, ch->sih,
	                       ...
	                       NULL /* rxq unused */,
	                       NTXD, 0, 0, ETC_DMARXHDROOM, 0, 0, &et_msg_level);
...
#define DMA_CTL_BL 3
	...
	if (et_phy_reset(etc)) { /* WICED: move PHY reset to WICED part of driver */
...
gmac_reset(ch_t *ch)
{
	...
	cmdcfg &= ~(CC_TE | CC_RE | CC_RPI | CC_TAI | CC_HD | CC_ML |
	            CC_CFE | CC_RL | CC_RED | CC_PE | CC_TPI | CC_PAD_EN | CC_PF | CC_PROM | CC_AT); /* WICED: CC_PROM and CC_AT in original code was set, in WICED let's clear it */
	cmdcfg |= (CC_NLC | CC_CFE | CC_TPI);
...
static void
gmac_macloopback(ch_t *ch, uint32 mode)
{
	uint32 ocmdcfg, cmdcfg;
	...
	if (mode == LOOPBACK_MODE_MAC_MII)
		cmdcfg |= CC_ML;
	else if (mode == LOOPBACK_MODE_MAC_LINE)
		cmdcfg |= CC_RL;
...
gmac_loopback(ch_t *ch, uint32 mode)
{
	switch (mode) {
		...
		case LOOPBACK_MODE_MAC_MII:
		case LOOPBACK_MODE_MAC_LINE:
		case LOOPBACK_MODE_PHY:
		case LOOPBACK_MODE_NONE:
			gmac_macloopback(ch, mode);
			chipphyloopback(ch, mode);
			break;
...
chipreset(ch_t *ch)
{
	...
	gmac_loopback(ch, LOOPBACK_MODE_MAC_MII);
	OSL_DELAY(1);
...
chipinreset:
	...
	if ((CHIPID(ch->sih->chip) == BCM5357_CHIP_ID) ||
		...
		if ((var = getvar(ch->vars, "et_swtype")) != NULL)
			sw_type = (bcm4390x_atoi(var) & 0x0f) << 4;
...
chipinit(ch_t *ch, uint options)
{
	...
	memcpy(&macaddrhigh, &etc->cur_etheraddr.octet[0], sizeof(macaddrhigh)); /* WICED: use memcpy() to get rid from strict-aliasing violation */
	memcpy(&macaddrlow, &etc->cur_etheraddr.octet[4], sizeof(macaddrlow));
	W_REG(ch->osh, &regs->macaddrhigh, hton32(macaddrhigh));
	W_REG(ch->osh, &regs->macaddrlow, hton16(macaddrlow));
...
chiprxquota(ch_t *ch, int quota, void **rxpkts)
{
	...
	/* dma_rx: prefetches HWRXOFF=30B for applying PR77137: len update chk */
...
static void chipphyloopback(ch_t *ch, uint32 mode)
{
	...
	ET_TRACE(("et%d: chipphyloopback: phyaddr %d\n",
	          ch->etc->unit, phyaddr));

	ctl = chipphyrd(ch, phyaddr, 0);

	if (mode == LOOPBACK_MODE_PHY) {
		new_ctl = ctl | CTL_LOOP;
	} else {
		new_ctl = ctl & ~CTL_LOOP;
	}

	if (new_ctl != ctl) {
		chipphywr(ch, phyaddr, 0, new_ctl);
	}
}
