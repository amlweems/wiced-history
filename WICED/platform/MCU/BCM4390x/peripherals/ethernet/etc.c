...
/* local prototypes */
static void etc_loopback(etc_info_t *etc, uint on);
...
static void
etc_loopback(etc_info_t *etc, uint mode)
{
	ET_TRACE(("et%d: etc_loopback: 0x%x\n", etc->unit, mode));
...
etc_dumpetc(etc_info_t *etc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "up %d promisc %d loopbk 0x%x forcespeed %d advertise 0x%x "
