...
static void hwcrypto_compute_sha_hmac_inner_outer_hashcontext( uint8_t* key, uint32_t keylen, uint8_t i_key_pad[ ], uint8_t o_key_pad[ ] );
...
static uint32_t     spum_header[ PLATFORM_L1_CACHE_ROUND_UP( SPUM_HDR_SIZE * BYTES_IN_WORD ) ] ALIGNED(HWCRYPTO_ALIGNMENT_BYTES);
...
static void populate_input_descriptors( uint32_t* total_size, crypto_cmd_t* cmd, uint32_t* num_input_descriptors)
{
     ...
     */
    if ( cmd->msg.sctx2.bits.hash_algo == SHA1   ||
         cmd->msg.sctx2.bits.hash_algo == SHA224 ||
         cmd->msg.sctx2.bits.hash_algo == SHA256 )
    {
        if ( ( ( input_descriptors[num_payload_descriptors].ctrl2 ) & SHA_BLOCK_SIZE_MASK ) != 0 )
        {
...
static void populate_hash_payload_padding(uint32_t* num_payload_descriptors, uint32_t* total_size)
{
     ...
     * actually 32 bits as HWCRPTO_MAX_PAYLOAD_SIZE < 2^32 */
    *( uint32_t* )( ( uint8_t* )buffer + ( ( num_padding_bits / BITS_IN_BYTE ) + 4 ) ) = (uint32_t)( *total_size * BITS_IN_BYTE);
     ...
     * num_padding_bytes
     */
    input_descriptors[ next_descriptor_index ].ctrl2    = unaligned_bytes + 8 + ( num_padding_bits / BITS_IN_BYTE );
...
platform_result_t platform_hwcrypto_execute( crypto_cmd_t cmd )
{
    ...
    memset( spum_header, 0, ( SPUM_HDR_SIZE * BYTES_IN_WORD ) );
    memset( status, 0, SPUM_STATUS_SIZE );

    /* Either cyrpto payload competely overlaps the hash payload
...
void hw_des3_set2key(hw_des_context_t* ctx, unsigned char* key)
{
    ...
    memcpy((unsigned char*)key+16, (unsigned char*)key, 8);
    ctx->key    = key;
    ctx->keylen = 192;
}
...
platform_result_t platform_hwcrypto_sha2_hmac_starts( sha256_context_t* ctx, const unsigned char* key, uint32_t keylen, int32_t is224 )
{
     ...
     */
    hwcrypto_compute_sha_hmac_inner_outer_hashcontext( (unsigned char *)key, keylen, ctx->i_key_pad, ctx->o_key_pad );
...
platform_result_t platform_hwcrypto_sha2_hmac_finish( sha256_context_t* ctx, uint8_t* input, int32_t ilen, unsigned char output[ SHA256_HASH_SIZE ] )
{
    crypto_cmd_t cmd =
    { 0 };
    uint8_t store_buffer[ HMAC_OUTERHASHCONTEXT_SIZE + HMAC256_OUTPUT_SIZE ] =
    { 0 };
    ...
    platform_hwcrypto_sha2( store_buffer, ( HMAC_OUTERHASHCONTEXT_SIZE + sha2_output_size ), output, ctx->is224 );
 ...
 * @param[in]   is224           0 : SHA256 1: SHA224
 */
platform_result_t platform_hwcrypto_sha2(const unsigned char* input, uint32_t ilen, unsigned char output[ SHA256_HASH_SIZE ], int32_t is224)
{
    platform_result_t   result;
    ...
    else
    {
        ...
        cmd.source                          = ( unsigned char* )input;
        cmd.output                          = ( unsigned char* )input;
