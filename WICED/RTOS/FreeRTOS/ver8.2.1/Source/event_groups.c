...
#if( configNO_MALLOC )
    PRIVILEGED_DATA static EventGroup_t EventGroup_array[ configMAX_EVENTGROUPS ];
#endif
...
#if( configNO_MALLOC )
static EventGroup_t* prvGetAvailableEventGroup( void )
{
    int i;
    for( i = 0; i < sizeof(EventGroup_array)/sizeof(EventGroup_array[0]); i++ )
    {
        if ( EventGroup_array[i].inuse == 0 )
        {
            EventGroup_array[i].inuse = 1;
            return &EventGroup_array[i];
        }
    }
    return NULL;
}
...
#if( configNO_MALLOC )
    pxEventBits = prvGetAvailableEventGroup( );
#else
