...
#if ( configUSE_NEWLIB_MALLOC_LOCK == 1 )
#include "semphr.h"
#endif /* configUSE_NEWLIB_MALLOC_LOCK */
...
#if( configNO_MALLOC )
    PRIVILEGED_DATA static TCB_t TCB_array[ configMAX_THREADS ];
    static StackType_t IdleTaskStaticStack[tskIDLE_STACK_SIZE];
#endif
...
#endif /* if ( configFREE_TASKS_IN_IDLE == 1 ) */
...
StackType_t *pxTopOfStack;
	...
	{
#if ( INCLUDE_vTaskForceAwake == 1 )
        pxNewTCB->forceAwakePending = pdFALSE;
#endif /* INCLUDE_vTaskForceAwake */
	...
	TCB_t *pxTCB;
		...
		if( pxTCB == pxCurrentTCB )
		{
			...
			else
			{
                ...
                unsigned portBASE_TYPE tmp_uxCurrentNumberOfTasks = uxCurrentNumberOfTasks; /* required to define order of access to volatile variables */
                if( listCURRENT_LIST_LENGTH( &xSuspendedTaskList ) == tmp_uxCurrentNumberOfTasks )
				{
...
#endif /* INCLUDE_xTaskGetCurrentThread */
/*-----------------------------------------------------------*/

#if ( INCLUDE_vTaskForceAwake == 1 )

    void vTaskForceAwake ( TaskHandle_t pxTaskToResume )
    {
        tskTCB* pxTCB = ( tskTCB * ) pxTaskToResume;
        taskENTER_CRITICAL();
        {
            if( pxTCB->xEventListItem.pvContainer )
            {
                /* remove a task from the event list, it may be blocked on a mutex or a queue or a semaphore */
                /* after it has been removed it is no longer subscribed to semaphore or queue events */
                uxListRemove( &( pxTCB->xEventListItem ) );
                /* remove it from the delayed list or suspended list and add it to the  */
                uxListRemove( &( pxTCB->xGenericListItem ) );
                prvAddTaskToReadyList( pxTCB );
                pxTCB->forceAwakePending = pdTRUE;
             }
         }
         taskEXIT_CRITICAL();
    }
...
/*-----------------------------------------------------------*/
    ...
    inline portBASE_TYPE xTaskForceAwakePending( void )
    {
        return pxCurrentTCB->forceAwakePending;
    }

/*-----------------------------------------------------------*/

    inline void xTaskForceAwakeClearPending( void )
    {
        pxCurrentTCB->forceAwakePending = pdFALSE;
    }
...
BaseType_t xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
        ...
        #if( configNO_MALLOC )
            xReturn = xTaskGenericCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle, IdleTaskStaticStack, NULL );
        #else
	...
	#else
	{
        ...
        #if( configNO_MALLOC )
	        xReturn = xTaskGenericCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL, IdleTaskStaticStack, NULL );
        #else
	...
	if( xReturn == pdPASS )
	{
        ...
        #if ( configUSE_NEWLIB_MALLOC_LOCK == 1 )
        {
            extern SemaphoreHandle_t newlib_malloc_mutex;
            /* Initialize mutex for newlib __malloc_lock / __malloc_unlock functions */
            newlib_malloc_mutex = xSemaphoreCreateRecursiveMutex( );
        }
        #endif /* configUSE_NEWLIB_MALLOC_LOCK */
...
TickType_t GetTimeTillNextWake( void )
{
	return (xNextTaskUnblockTime==0xffffffff)? 0xffffffff : xNextTaskUnblockTime - xTickCount;
}

void AddMissedTicks( TickType_t ticks )
{
    uxPendedTicks += ticks;
}
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
        ...
        #if ( configUSE_IDLE_SLEEP_HOOK == 1 )
        {
            extern void platform_idle_hook( void );

            /* Call the user defined function from within the idle task.  This
            allows the processor sleep when idle - system tick must remain active */
            platform_idle_hook();
        }
        #endif /* configUSE_IDLE_SLEEP_HOOK */

        #if ( configUSE_IDLE_NO_TICK_SLEEP_HOOK == 1 )
        {
            extern unsigned portBASE_TYPE platform_power_down_hook( TickType_t MaxSleepTime );

            /* Call the user defined function from within the idle task.  This
            allows the processor sleep when idle, including switching off the system tick */
            vTaskSuspendAll();
            uxPendedTicks += platform_power_down_hook( GetTimeTillNextWake( ) );
            xTaskResumeAll();
		}
		#endif /* configUSE_IDLE_NO_TICK_SLEEP_HOOK */
...
{
    char *              out = pxTCB->pcTaskName;
    const char *        in  = pcName;

    /* Store the task name in the TCB.
     * Don't copy all configMAX_TASK_NAME_LEN if the string is shorter than
     * configMAX_TASK_NAME_LEN characters - avoids an array index out of bounds warning
     * and possible hang if the memory after the string is not accessible
     */
    for ( ; ( *in != 0x00 ) && ( in < (pcName + configMAX_TASK_NAME_LEN - 1 )); in++, out++ )
    {
        *out = *in;
    }
    *out = 0;
	...
	if( uxPriority >= ( UBaseType_t ) uxTopUsedPriority )  /* WICED: Changed to uxTopUsedPriority from configMAX_PRIORITIES to ensure the uxTopUsedPriority is always linked - enabling OpenOCD to work properly */
	{
...
#else /* if ( configFREE_TASKS_IN_IDLE == 1 ) */

portBASE_TYPE vTaskFreeTerminated( TaskHandle_t xTask )
{
    tskTCB * pxTCB = prvGetTCBFromHandle( xTask );

    if ( listIS_CONTAINED_WITHIN( &xTasksWaitingTermination, &( pxTCB->xGenericListItem ) ) != pdTRUE )
    {
        return pdFALSE;
    }
...
{
    tskTCB * tmp_pxCurrentTCB = pxCurrentTCB;

    /* The list item will be inserted in wake time order. */
	listSET_LIST_ITEM_VALUE( &( tmp_pxCurrentTCB->xGenericListItem ), xTimeToWake );

	if( xTimeToWake < xTickCount )
	{
		/* Wake time has overflowed.  Place this item in the overflow list. */
		vListInsert( pxOverflowDelayedTaskList, &( tmp_pxCurrentTCB->xGenericListItem ) );
	}
	else
	{
		/* The wake time has not overflowed, so the current block list is used. */
		vListInsert( pxDelayedTaskList, &( tmp_pxCurrentTCB->xGenericListItem ) );
...
/*-----------------------------------------------------------*/
#if( configNO_MALLOC )
static TCB_t* prvGetAvailableTCB( void )
{
    int i;
    for( i = 0; i < sizeof(TCB_array)/sizeof(TCB_array[0]); i++ )
    {
        if ( TCB_array[i].inuse == 0 )
        {
            TCB_array[i].inuse = 1;
            return &TCB_array[i];
        }
...
#if ( ( configUSE_TRACE_FACILITY == 1 ) && ( configUSE_STATS_FORMATTING_FUNCTIONS > 0 ) && ( ! configNO_MALLOC ) )
...
#if ( ( configGENERATE_RUN_TIME_STATS == 1 ) && ( configUSE_STATS_FORMATTING_FUNCTIONS > 0 ) && ( ! configNO_MALLOC ))
