...
{
    return (xTaskHandle) pxCurrentTCB;
}


void vTaskForceAwake ( xTaskHandle pxTaskToResume )
{
...
void vTaskGetStackInfo( xTaskHandle pxTask, void** stack_ptr, void** start_stack_ptr, void** end_stack_ptr )
{
    taskENTER_CRITICAL();
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
        ...
        {
            unsigned portBASE_TYPE tmp_xTickCount;
            extern unsigned portBASE_TYPE platform_power_down_hook( portTickType MaxSleepTime );
            ...
            vTaskSuspendAll();
            prvCheckDelayedTasks();
            tmp_xTickCount = xTickCount;
            uxMissedTicks += platform_power_down_hook( (xNextTaskUnblockTime==0xffffffff)? 0xffffffff : xNextTaskUnblockTime - tmp_xTickCount );
            xTaskResumeAll();
        }
        #endif
	}
...
static void prvAddCurrentTaskToDelayedList( portTickType xTimeToWake )
{
	...
	if( xTimeToWake < xTickCount )
	{
		...
		vListInsert( ( xList * ) pxOverflowDelayedTaskList, ( xListItem * ) &( tmp_pxCurrentTCB->xGenericListItem ) );
	}
	else
	{
		/* The wake time has not overflowed, so we can use the current block list. */
		vListInsert( ( xList * ) pxDelayedTaskList, ( xListItem * ) &( tmp_pxCurrentTCB->xGenericListItem ) );
