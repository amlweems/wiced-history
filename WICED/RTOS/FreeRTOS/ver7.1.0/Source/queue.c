	...
	for( ;; )
	{
		...
		if( xTaskCheckForTimeOut( &xTimeOut, &xTicksToWait ) == pdFALSE )
		{
			if( prvIsQueueEmpty( pxQueue ) != pdFALSE )
			{
				...
				{
                    portYIELD_WITHIN_API();
	                /* WICED_CHANGES_FREERTOS for forceThreadAwake support */
	                prvLockQueue( pxQueue );
	                ...
	                /* a forceAwake function will deattach task from the queue and place it from suspended or delayed list */
