...
typedef enum
{
    ...
    WLC_E_ASSOC_REQ_IE                   = 85,
    WLC_E_ASSOC_RESP_IE                  = 86,

    WLC_E_LAST                           = 87, /** highest val + 1 for range checking */
...
typedef struct
{
    uint16_t             reserved;
    uint16_t             flags;
    wiced_event_num_t    event_type;
    wiced_event_status_t status;
    wiced_event_reason_t reason;
    uint32_t             auth_type;
    uint32_t             datalen;
    wiced_mac_t          addr;
    uint8_t              interface;
} wiced_event_header_t;
#pragma pack()

/**
 * Event handler prototype definition
 */
typedef void* (*wiced_event_handler_t)( wiced_event_header_t* event_header, uint8_t* event_data, /*@null@*/ void* handler_user_data );
 ...
 *                        See @ref wiced_event_num_t for available events
...
extern wiced_result_t wiced_management_set_event_handler( const wiced_event_num_t* event_nums, /*@null@*/ wiced_event_handler_t handler_func, /*@null@*/ void* handler_user_data );
extern wiced_result_t wwd_management_set_event_handler( const wiced_event_num_t* event_nums, /*@null@*/ wiced_event_handler_t handler_func, /*@null@*/ void* handler_user_data, wiced_interface_t interface );
