...
wiced_result_t wiced_time_get_iso8601_time(wiced_iso8601_time_t* iso8601_time)
{
    ...
    sub_second = (( utc_time_ms % 1000 ) * 1000 ); /* Sub-second is in microseconds               */

    /* Calculate year */
    year = 1970 + second / SECONDS_IN_365_DAY_YEAR;
    number_of_leap_years = ( year - 1968 - 1 ) / 4;
    second = second - ( ( year - 1970 ) * SECONDS_IN_365_DAY_YEAR );
    if ( second > ( number_of_leap_years * SECONDS_IN_A_DAY ) )
    {
        second = second - ( number_of_leap_years * SECONDS_IN_A_DAY );
    }
    else
    {
        second = second - ( number_of_leap_years * SECONDS_IN_A_DAY ) + SECONDS_IN_365_DAY_YEAR;
        --year;
    }
    ...
    /* Calculate day */
    day    = second / SECONDS_IN_A_DAY;
    second = second - ( day * SECONDS_IN_A_DAY );
    ++day;

    /* Calculate hour */
    hour   = second / SECONDS_IN_A_HOUR;
    second = second - ( hour * SECONDS_IN_A_HOUR );

    /* Calculate minute */
    minute = second / SECONDS_IN_A_MINUTE;
    second = second - ( minute * SECONDS_IN_A_MINUTE );
    ...
    utoa( second,     iso8601_time->second,     2, 2 );
