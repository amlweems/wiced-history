...
ARM_GNU_ARCH_LIST := ARM_Cortex_M3 \
                     ARM_Cortex_M4
...
GDBINIT_STRING     = shell start /B $(OPENOCD_FULL_NAME) -f $(OPENOCD_PATH)$(JTAG).cfg -f $(OPENOCD_PATH)$(HOST_MICRO).cfg -f $(OPENOCD_PATH)$(HOST_MICRO)_gdb_jtag.cfg -l $(OPENOCD_LOG_FILE)
...
GDBINIT_STRING     = 'shell $(COMMON_TOOLS_PATH)dash -c "trap \\"\\" 2;$(OPENOCD_FULL_NAME) -f $(OPENOCD_PATH)$(JTAG).cfg -f $(OPENOCD_PATH)$(HOST_MICRO).cfg -f $(OPENOCD_PATH)$(HOST_MICRO)_gdb_jtag.cfg -l $(OPENOCD_LOG_FILE) &"'
...
ADD_COMPILER_SPECIFIC_STANDARD_CFLAGS   = $(1) -Wall -fsigned-char -ffunction-sections -Werror -fdata-sections -fno-common
ADD_COMPILER_SPECIFIC_STANDARD_CXXFLAGS = $(1) -Wall -fsigned-char -ffunction-sections -Werror -fdata-sections -fno-common -fno-rtti -fno-exceptions
ADD_COMPILER_SPECIFIC_STANDARD_ADMFLAGS = $(1)
COMPILER_SPECIFIC_PEDANTIC_CFLAGS  := $(COMPILER_SPECIFIC_STANDARD_CFLAGS) -Wstrict-prototypes  -W -Wshadow  -Wwrite-strings -pedantic -std=c99 -Wconversion -Wextra -Wdeclaration-after-statement -Wconversion -Waddress -Wlogical-op -Wstrict-prototypes -Wold-style-definition -Wmissing-prototypes -Wmissing-declarations -Wmissing-field-initializers -Wdouble-promotion -Wswitch-default -Wuninitialized -Wunknown-pragmas -Wfloat-equal  -Wundef  -Wshadow # -Wcast-qual -Wtraditional -Wtraditional-conversion
COMPILER_SPECIFIC_ARFLAGS_CREATE   := -rcs
COMPILER_SPECIFIC_ARFLAGS_ADD      := -rcs
COMPILER_SPECIFIC_ARFLAGS_VERBOSE  := -v
COMPILER_SPECIFIC_DEBUG_CFLAGS     := -DDEBUG -ggdb
COMPILER_SPECIFIC_DEBUG_CXXFLAGS   := -DDEBUG -ggdb
COMPILER_SPECIFIC_DEBUG_ASFLAGS    := --defsym DEBUG=1 -ggdb
COMPILER_SPECIFIC_DEBUG_LDFLAGS    := -Wl,--gc-sections -Wl,--cref
COMPILER_SPECIFIC_RELEASE_CFLAGS   := -DNDEBUG -ggdb
COMPILER_SPECIFIC_RELEASE_CXXFLAGS := -DNDEBUG -ggdb
...
COMPILER_SPECIFIC_OPTIMISE_CFLAGS    := -O3
COMPILER_SPECIFIC_OPTIMISE_CXXFLAGS  := -O3
COMPILER_SPECIFIC_UNOPTIMISE_CFLAGS  := -O0
COMPILER_SPECIFIC_UNOPTIMISE_CXXFLAGS:= -O0
LINKER                             := $(CXX) --static -Wl,-static -Wl,--warn-common
TOOLCHAIN_NAME := GCC
OPTIONS_IN_FILE_OPTION    := @

# Chip specific flags for GCC
ifeq ($(HOST_ARCH),ARM_Cortex_M4)
...
COMPILER_SPECIFIC_DEBUG_CFLAGS        := -DDEBUG --debug
COMPILER_SPECIFIC_DEBUG_CXXFLAGS      := -DDEBUG --debug
COMPILER_SPECIFIC_DEBUG_ASFLAGS       :=
COMPILER_SPECIFIC_DEBUG_LDFLAGS       :=
COMPILER_SPECIFIC_RELEASE_CFLAGS      := -DNDEBUG --debug
COMPILER_SPECIFIC_RELEASE_CXXFLAGS    := -DNDEBUG --debug
...
COMPILER_SPECIFIC_OPTIMISE_CFLAGS    := -Oh
COMPILER_SPECIFIC_OPTIMISE_CXXFLAGS  := -Oh
COMPILER_SPECIFIC_UNOPTIMISE_CFLAGS  := -On
COMPILER_SPECIFIC_UNOPTIMISE_CXXFLAGS:= -On
...
endif #ifneq ($(filter $(HOST_ARCH), ARM_Cortex_M3 ARM_Cortex_M4),)
