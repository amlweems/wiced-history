...
wiced_result_t wiced_bt_rfcomm_init( void )
{
    if ( rfcomm_instance.initialised == WICED_FALSE )
    {
        ...
        if ( wiced_rtos_init_mutex( &rfcomm_instance.shared.mutex ) != WICED_SUCCESS )
        {
        ...
        if ( bt_mpaf_register_rfcomm_callback( rfcomm_mpaf_event_handler, rfcomm_mpaf_rx_data_handler ) != WICED_SUCCESS )
        {
...
wiced_result_t wiced_bt_rfcomm_deinit( void )
{
    ...
    {
        /* Deinit MPAF */
        if ( bt_mpaf_register_rfcomm_callback( NULL, NULL ) != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ("Error deregistering MPAF RFCOMM callback\r\n") );
            return WICED_ERROR;
        }

        /* Deinit access mutex */
        if ( wiced_rtos_deinit_mutex( &rfcomm_instance.shared.mutex ) != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ("Error deinitting RFCOMM data mutex\r\n") );
            return WICED_ERROR;
        }
...
wiced_result_t wiced_bt_rfcomm_listen( wiced_bt_rfcomm_socket_t* socket, wiced_bool_t auto_accept, const char* service_name, const wiced_bt_uuid_t* uuid )
{
    ...
    if ( bt_mpaf_rfcomm_create_connection( (const void*)&uuid->value, (uuid_size_t)uuid->size, wiced_bt_device_get_name(), MPAF_RFCOMM_SERVER_MODE, FEATURE_MASK ) != WICED_SUCCESS )
    {
    ...
    if ( params->command_status.status == MPAF_SUCCESS )
    {
        ...
        if ( FEATURE_MASK == MPAF_RFCOMM_SERVICE_CHANNEL_NUMBER_MASK )
        {
            ...
            {
                if ( rfcomm_add_sdp_record( socket, service_name, uuid ) != WICED_SUCCESS )
                {
                    goto error;
                }
...
wiced_result_t wiced_bt_rfcomm_connect( wiced_bt_rfcomm_socket_t* socket, const char* service_name, const wiced_bt_uuid_t* uuid )
{
    ...
    /* Create RFCOMM connection. This is an asynchronous function. The result will returned via rfcomm_mpaf_event_handler */
    if ( bt_mpaf_rfcomm_create_connection( (const void*)&uuid->value, (uuid_size_t)uuid->size, wiced_bt_device_get_name(), MPAF_RFCOMM_CLIENT_MODE, 0 ) != WICED_SUCCESS )
    {
        WPRINT_LIB_ERROR( ("Error creating RFCOMM connection\r\n") );
        goto error;
    }
...
wiced_result_t wiced_bt_rfcomm_disconnect( wiced_bt_rfcomm_socket_t* socket )
{
    ...
    if ( ( status & RFCOMM_SOCKET_LISTENING ) != 0 || ( status & RFCOMM_SOCKET_CONNECTING ) != 0 )
    {
        ...
        if ( bt_mpaf_rfcomm_create_connection_cancel() != WICED_SUCCESS )
        {
    ...
    {
        /* Disconnect RFCOMM This is an asynchronous function. The result will returned via rfcomm_mpaf_event_handler */
        if ( bt_mpaf_rfcomm_remove_connection( endpoint ) != WICED_SUCCESS )
        {
...
wiced_result_t wiced_bt_rfcomm_init_socket( wiced_bt_rfcomm_socket_t* socket, wiced_bt_rfcomm_event_handler_t handler, void* arg )
{
    if ( socket->id != RFCOMM_SOCKET_ID )
    {
        ...
        if ( wiced_rtos_init_mutex( &socket->shared.mutex ) != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ("Error initting RFCOMM socket mutex\r\n") );
            return WICED_ERROR;
        }

        if ( bt_linked_list_init( &socket->shared.rx_packet_list ) != WICED_SUCCESS )
        {
...
//            {
//                if ( rfcomm_delete_sdp_record( socket ) != WICED_SUCCESS )
//                {
//                    return WICED_ERROR;
//                }
//            }
//        }
        ...
        {
            WPRINT_LIB_ERROR( ("Error deinitting RFCOMM socket semaphore\r\n") );
            return WICED_ERROR;
        }

        if ( wiced_rtos_deinit_mutex( &socket->shared.mutex ) != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ("Error deinitting RFCOMM socket mutex\r\n") );
            return WICED_ERROR;
        }

        if ( bt_linked_list_deinit( &socket->shared.rx_packet_list ) != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ("Error deinitting RX packet list\r\n") );
            return WICED_ERROR;
        }
...
static void rfcomm_mpaf_rx_data_handler( uint8_t endpoint, bt_packet_t* packet, uint8_t* data_start, uint32_t data_size )
{
    ...
    if ( wiced_rtos_set_semaphore( &socket->semaphore ) != WICED_SUCCESS )
    {
        ...
        case MPAF_COMMAND_COMPLETE:
        {
            switch ( params->command_complete.command_opcode )
            {
                case MPAF_RFCOMM_CREATE_CONNECTION_CANCEL:
                    ...
                    if ( bt_management_mpaf_notify_event_received( packet ) != WICED_SUCCESS )
                    {
