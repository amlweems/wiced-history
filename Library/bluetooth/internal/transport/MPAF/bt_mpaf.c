...
wiced_result_t bt_mpaf_init( mpaf_event_cb_t system_event_cb )
{
    if ( mpaf_initialised == WICED_FALSE )
    {
        ...
        if ( bt_transport_driver_init( bt_transport_driver_event_handler, bt_transport_driver_bus_read_handler ) != WICED_SUCCESS )
        {
        ...
        if ( wiced_rtos_init_mutex( &mpaf_command_mutex ) != WICED_SUCCESS )
        {
        ...
        if ( bt_transport_thread_init( bt_transport_thread_received_packet_handler ) != WICED_SUCCESS )
        {
...
wiced_result_t bt_mpaf_deinit( void )
{
    if ( mpaf_initialised == WICED_TRUE )
    {
        ...
        if ( wiced_rtos_deinit_mutex( &mpaf_command_mutex ) != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ( "Error creating MPAF command mutex\r\n" ) );
            return WICED_ERROR;
        }
        ...
        {
            WPRINT_LIB_ERROR( ( "Error deinitialising MPAF driver\r\n" ) );
            return WICED_ERROR;
        }

        /* Deinitialise packet pools */
        result = bt_packet_pool_deinit( &mpaf_command_packet_pool );

        if ( result != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ( "Error deinitialising MPAF command packet pool\r\n" ) );
            return result;
        }

        result = bt_packet_pool_deinit( &mpaf_event_packet_pool );

        if ( result != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ( "Error deinitialising MPAF event packet pool\r\n" ) );
            return result;
        }

        result = bt_packet_pool_deinit( &mpaf_data_packet_pool );

        if ( result != WICED_SUCCESS )
        {
            WPRINT_LIB_ERROR( ( "Error deinitialising MPAF data packet pool\r\n" ) );
            return result;
        }
...
static wiced_result_t bt_mpaf_application_callback( void* arg )
{
    ...
    if ( common_packet->header.header.endpoint == 0 )
    {
        ...
        if ( event_packet->header.opcode == MPAF_COMMAND_COMPLETE )
        {
            ...
            {
                if ( mpaf_sdp_event_callback != NULL )
                {
            ...
            {
                if ( mpaf_rfcomm_event_callback != NULL )
                {
    ...
    else
    {
        ...
        if ( mpaf_rx_data_packet_callback != NULL )
        {
