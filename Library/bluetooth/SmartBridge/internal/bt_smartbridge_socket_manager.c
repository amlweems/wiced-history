...
{
    if ( bt_linked_list_init( &connected_socket_list ) != WICED_SUCCESS )
    {
        WPRINT_LIB_INFO( ( "Error creating linked list\r\n" ) );
        return WICED_ERROR;
    }

    if ( wiced_rtos_init_mutex( &connected_socket_list_mutex ) != WICED_SUCCESS )
    {
...
{
    if ( wiced_rtos_deinit_mutex( &connected_socket_list_mutex ) != WICED_SUCCESS )
    {
        WPRINT_LIB_INFO( ( "Error deinitialising mutex\r\n" ) );
        return WICED_ERROR;
    }

    if ( bt_linked_list_deinit( &connected_socket_list ) != WICED_SUCCESS )
    {
        WPRINT_LIB_INFO( ( "Error deinitialising linked list\r\n" ) );
        return WICED_ERROR;
    }
...
wiced_result_t bt_smartbridge_socket_manager_insert_socket( wiced_bt_smartbridge_socket_t* socket )
{
    ...
    /* Lock protection */
    if ( wiced_rtos_lock_mutex( &connected_socket_list_mutex ) != WICED_SUCCESS )
    {
        return WICED_ERROR;
    }
    ...
    /* Unlock protection */
    if ( wiced_rtos_unlock_mutex( &connected_socket_list_mutex ) != WICED_SUCCESS )
    {
        return WICED_ERROR;
    }
