...
wiced_result_t wiced_bt_smartbridge_init( void )
{
    if ( initialised == WICED_FALSE )
    {
        ...
        if ( bt_smart_gatt_init() != WICED_SUCCESS )
        {
        ...
        if ( bt_smart_gap_init() != WICED_SUCCESS )
        {
        ...
        if ( bt_smartbridge_socket_manager_init() != WICED_SUCCESS )
        {
...
wiced_result_t wiced_bt_smartbridge_deinit( void )
{
    if ( initialised == WICED_TRUE )
    {
        ...
        if ( bt_smartbridge_att_cache_disable() != WICED_SUCCESS )
        {
            WPRINT_LIB_INFO( ( "Error deinitialising SmartBridge Attribute Cache\r\n" ) );
            return WICED_ERROR;
        }

        /* Deinitialise socket manager */
        if ( bt_smartbridge_socket_manager_deinit() != WICED_SUCCESS )
        {
            WPRINT_LIB_INFO( ( "Error deinitialising SmartBridge Socket Manager\r\n" ) );
            return WICED_ERROR;
        }

        /* Deinitialise GATT */
        if ( bt_smart_gatt_deinit() != WICED_SUCCESS )
        {
            WPRINT_LIB_INFO( ( "Error deinitialising GATT\r\n" ) );
            return WICED_ERROR;
        }
        ...
        /* Deinitialise GAP */
        if ( bt_smart_gap_deinit() != WICED_SUCCESS )
        {
            WPRINT_LIB_INFO( ( "Error deinitialising GAP\r\n" ) );
            return WICED_ERROR;
        }
...
wiced_result_t wiced_bt_smartbridge_create_socket( wiced_bt_smartbridge_socket_t* socket )
{
    ...
    if ( wiced_rtos_init_semaphore( &socket->semaphore ) != WICED_SUCCESS )
    {
        return WICED_ERROR;
    }
...
wiced_result_t wiced_bt_smartbridge_delete_socket( wiced_bt_smartbridge_socket_t* socket )
{
    ...
    if ( wiced_rtos_deinit_semaphore( &socket->semaphore ) != WICED_SUCCESS )
    {
        return WICED_ERROR;
    }
...
wiced_result_t wiced_bt_smartbridge_connect( wiced_bt_smartbridge_socket_t* socket, const wiced_bt_smart_device_t* remote_device, const wiced_bt_smart_connection_settings_t* settings, wiced_bt_smartbridge_disconnection_callback_t disconnection_callback, wiced_bt_smartbridge_notification_callback_t notification_callback )
{
    ...
    if ( bt_smartbridge_socket_manager_find_socket_by_address( &remote_device->address, &found_socket ) == WICED_SUCCESS )
    {
    ...
    while( wiced_rtos_get_semaphore( &socket->semaphore, WICED_NO_WAIT ) == WICED_SUCCESS )
    {
    }
    ...
    if ( smartbridge_socket_check_actions_enabled( socket, SOCKET_ACTION_INITIATE_PAIRING ) == WICED_TRUE )
    {
        ...
        bt_smart_gap_enable_pairing( &socket->security_settings );
    }
    else
    {
    ...
    if ( bt_smartbridge_att_cache_is_enabled() == WICED_TRUE )
    {
        ...
        if ( bt_smartbridge_att_cache_find( remote_device, &cache ) == WICED_SUCCESS )
        {
        ...
        else
        {
            ...
            if ( bt_smartbridge_att_cache_generate( remote_device, socket->connection_handle, &cache ) != WICED_SUCCESS )
            {
                goto error;
            }
        }
...
wiced_result_t wiced_bt_smartbridge_enable_pairing( wiced_bt_smartbridge_socket_t* socket, const wiced_bt_smart_security_settings_t* settings, wiced_bt_smartbridge_pairing_callback_t pairing_callback )
{
    if ( initialised == WICED_FALSE )
    {
        return WICED_NOTREADY;
    ...
    socket->pairing_callback = pairing_callback;
...
wiced_result_t wiced_bt_smartbridge_enable_attribute_cache_notification( wiced_bt_smartbridge_socket_t* socket )
{
    ...
    if ( initialised == WICED_FALSE || bt_smartbridge_att_cache_is_enabled( ) == WICED_FALSE )
    {
        return WICED_NOTREADY;
    }
...
static wiced_result_t smartbridge_gap_connection_handler( bt_smart_gap_connection_event_t event, wiced_bt_device_address_t* address, wiced_bt_smart_address_type_t type, uint16_t connection_handle )
{
    switch ( event )
    {
        ...
        case GAP_DISCONNECTED:
        {
            ...
            if ( bt_smartbridge_socket_manager_remove_socket( connection_handle, &removed_socket ) == WICED_SUCCESS )
            {
                /* Reset connection handle to invalid value */
...
static wiced_result_t smartbridge_gatt_indication_notification_handler( uint16_t connection_handle, uint16_t attribute_handle, uint8_t* data, uint16_t length )
{
    ...
    if ( bt_smartbridge_socket_manager_find_socket_by_handle( connection_handle, &socket ) == WICED_SUCCESS )
    {
        if ( bt_smartbridge_att_cache_is_enabled() == WICED_TRUE && socket->att_cache != NULL )
        {
            ...
            if ( wiced_bt_smart_attribute_search_list_by_handle( att_cache_list, attribute_handle, &att ) == WICED_SUCCESS )
            {
...
static wiced_result_t smartbridge_gap_bonding_handler( uint16_t connection_handle, const wiced_bt_smart_bond_info_t* bond_info )
{
    ...
    if ( bt_smartbridge_socket_manager_find_socket_by_handle( connection_handle, &socket ) == WICED_SUCCESS )
    {
        ...
        if ( socket != NULL && socket->pairing_callback != NULL )
        {
...
static wiced_result_t smartbridge_app_pairing_handler( void* arg )
{
    ...
    if ( socket != NULL && socket->pairing_callback != NULL )
    {
        socket->pairing_callback( socket, &socket->bond_info );
        return WICED_SUCCESS;
    }
