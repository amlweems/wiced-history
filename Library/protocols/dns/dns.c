...
wiced_result_t dns_client_hostname_lookup( const char* hostname, wiced_ip_address_t* address, uint32_t timeout_ms )
{
    ...
    int32_t                remaining_time         = timeout_ms;
    wiced_ip_address_t     resolved_ipv4_address  = { WICED_INVALID_IP };
    wiced_ip_address_t     resolved_ipv6_address  = { WICED_INVALID_IP };
    ...
    while ( ipv4_address_found == WICED_FALSE && remaining_time > 0)
    {
        ...
        while ( ipv4_address_found == WICED_FALSE && remaining_time > 0 )
        {
                    ...
                    for ( a = 0; answer_found == WICED_FALSE && a < htobe16( iter.header->answer_count ); ++a )
                    {
                        ...
                        switch ( record.type )
                        {
                            case RR_TYPE_CNAME:
                                ...
                                hostname_length = strlen( temp_hostname );
            ...
            remaining_time = remaining_time - (int32_t) ( current_time - last_time );
        }
    }
...
wiced_bool_t dns_compare_name_to_string(dns_name_t* name, const char* string)
{
...
char* dns_read_name(uint8_t* data, dns_message_header_t* start_of_packet)
{
    uint16_t length = 0;
    uint8_t* buffer = data;
    ...
    char* string = malloc_named("dns", length);
...
void dns_write_record(dns_message_iterator_t* iter, const char* name, uint16_t class, uint16_t type, uint32_t TTL, const void* rdata)
{
    ...
    switch (type)
    {
        ...
        case RR_TYPE_NSEC:
        {
            ...
            iter->iter = dns_write_name(iter->iter, name);
            dns_write_bytes(iter, &nsec_data->block_number, 2 + nsec_data->bitmap_size);
        }
            break;
    ...
    rd_length[0] = (iter->iter - tempPtr) >> 8;
    rd_length[1] = (iter->iter - tempPtr) & 0xFF;
}
...
uint16_t dns_read_uint16(uint8_t* data)
{
    return htons(*((uint16_t*)data));
}

uint32_t dns_read_uint32(uint8_t* data)
{
