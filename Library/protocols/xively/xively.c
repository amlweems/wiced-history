...
wiced_result_t xively_open_feed( xively_feed_t* feed )
{
    ...
    wiced_tls_init_simple_context( &feed->tls_context );
...
void xively_u16toa( uint16_t value, char* output, uint8_t min_length )
{
    ...
    if ( min_length <= 5 )
    {
        digits_left = MIN(5-min_length, digits_left);
    }

    memcpy( output, &buffer[ digits_left ], 5 - digits_left );
}
...
wiced_result_t xively_create_datastream( xively_feed_t* feed, xively_datastream_t* stream, const char* channel_name, uint32_t data_size, uint32_t number_of_datapoints )
{
    ...
    content_length = number_of_datapoints * ( strlen( channel_name ) + ( sizeof( COMMA ) - 1 ) + sizeof(wiced_iso8601_time_t) + ( sizeof( COMMA ) - 1 ) + data_size + ( sizeof( NEWLINE ) - 1 ) );
