...
struct wiced_email_internal
{
    char date[32]; /* Date string in email header format */
};

typedef struct wiced_email_internal wiced_email_internal_t;
...
static void           init_content_table          ( const wiced_email_account_t *account, const wiced_email_t *email, email_content_table_field_t *content_table );
extern int32_t        base64_decode               ( unsigned char *dst, int32_t *dlen, unsigned char *src, int32_t slen );
extern int32_t        base64_encode               ( unsigned char *dst, int32_t *dlen, unsigned char *src, int32_t slen );
...
{
    malloc_transfer_to_curr_thread(account->internal);
    free( account->internal );
    account->internal = 0;
    malloc_thread_leak_check();
    return WICED_SUCCESS;
}

wiced_result_t wiced_smtp_send( wiced_email_account_t* account, wiced_email_t* email )
{
    ...
    email->internal = (wiced_email_internal_t*) MALLOC_OBJECT("email_int", wiced_email_internal_t);

    if ( email->internal == NULL )
    {
        return WICED_ERROR;
    }

    memset( email->internal, 0, sizeof( *( email->internal ) ) );
    ...
    snprintf( email->internal->date,
              sizeof( email->internal->date ),
    ...
    if ( strstr( reply, smtp_auth_login ) )
    {
        ...
        if ( strstr( buffer, smtp_username_prompt ) )
        {
            /* Encode username */
            INIT_BUFFER_AND_LENGTH(buffer, buffer_length);
            base64_encode( (uint8_t*) buffer, &buffer_length, (uint8_t*) account->user_name, strlen( account->user_name ) );

            VERIFY(WICED_SUCCESS, send_smtp_command(&account->internal->smtp_socket, buffer, buffer_length, NULL, 0));
            VERIFY(WICED_SUCCESS, receive_smtp_reply(&account->internal->smtp_socket, &packet, &reply, &length));
            VERIFY(SMTP_MORE_INFO, get_smtp_reply_code(&account->internal->smtp_socket, packet));
            ...
            if ( strstr( buffer, smtp_password_prompt ) )
            {
                ...
                INIT_BUFFER_AND_LENGTH(buffer, buffer_length);
                base64_encode( (uint8_t*) buffer, &buffer_length, (uint8_t*) account->password, strlen( account->password ) );
    ...
    VERIFY(WICED_SUCCESS, send_smtp_sender(&account->internal->smtp_socket, account->email_address, strlen(account->email_address)));
    VERIFY(WICED_SUCCESS, receive_smtp_reply(&account->internal->smtp_socket, &packet, &reply, &length));
    VERIFY(SMTP_ACTION_COMPLETED, get_smtp_reply_code(&account->internal->smtp_socket, packet));;
    wiced_packet_delete( packet );
    ...
    init_content_table(account, email, content_table);
    ...
    for ( field_index = 0; field_index < CONTENT_TABLE_SIZE; field_index++ )
    {
        for ( content_index = 0; content_index < content_table[field_index].length; content_index++ )
        {
            ...
            *data_ptr = content_table[field_index].content[content_index];
            ...
            /* Check if last bytes */
            if ( ( field_index == CONTENT_TABLE_SIZE - 1 ) && ( content_index == content_table[field_index].length - 1 ) )
            {
                /* Send packet. */
...
static wiced_result_t send_smtp_command( wiced_tcp_socket_t *socket, const char *command, uint8_t command_length, const char* params, uint16_t params_length )
{
    ...
    WICED_VERIFY( wiced_packet_create_tcp( socket, command_length + params_length, &packet, &data, &available_space ) );
...
static void init_content_table(const wiced_email_account_t *account, const wiced_email_t *email, email_content_table_field_t *content_table )
{
    ...
    content_table[index].content   = (uint8_t*)&email->internal->date;
    content_table[index++].length  = strlen(email->internal->date);
    content_table[index].content   = (uint8_t*)smtp_return_newline;
    content_table[index++].length  = GET_CONST_BUF_LENGTH(smtp_return_newline);
