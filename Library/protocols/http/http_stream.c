...
wiced_result_t http_extract_header( const char* header, char* value, int sz, wiced_packet_t* packet )
{
    uint8_t* data;
...
wiced_result_t http_extract_headers( wiced_packet_t* packet, http_header_t* headers, uint16_t number_of_headers )
{
    ...
    do
    {
            ...
            !(header_location[0] == '\r' && header_location[1] == '\n'))
        {
            ...
            {
                int string_length = strlen( headers[a].name );
...
wiced_result_t http_get_body( wiced_packet_t* packet, uint8_t** body, uint16_t* body_length )
{
    uint8_t* packet_data;
    ...
    *body_length = packet_data_length - ( ( start_of_body - (char*) packet_data ) );
