 ...
 ******************************************************/
static wiced_result_t filex_shim_init             ( void );
static wiced_result_t filex_shim_mount            ( wiced_block_device_t* device, wiced_filesystem_t* fs_handle_out );
static wiced_result_t filex_shim_unmount          ( wiced_filesystem_t* fs_handle );
static wiced_result_t filex_shim_file_get_details ( wiced_filesystem_t* fs_handle, const char* filename, wiced_dir_entry_details_t* details_out );
static wiced_result_t filex_shim_file_open        ( wiced_filesystem_t* fs_handle, wiced_file_t* file_handle_out, const char* filename, wiced_filesystem_open_mode_t mode );
static wiced_result_t filex_shim_file_seek        ( wiced_file_t* file_handle, int64_t offset, wiced_filesystem_seek_type_t whence );
static wiced_result_t filex_shim_file_tell        ( wiced_file_t* file_handle, uint64_t* location );
static wiced_result_t filex_shim_file_read        ( wiced_file_t* file_handle, void* data, uint64_t bytes_to_read, uint64_t* returned_bytes_count );
static wiced_result_t filex_shim_file_write       ( wiced_file_t* file_handle, const void* data, uint64_t bytes_to_write, uint64_t* written_bytes_count );
static wiced_result_t filex_shim_file_flush       ( wiced_file_t* file_handle );
static int            filex_shim_file_end_reached ( wiced_file_t* file_handle );
static wiced_result_t filex_shim_file_close       ( wiced_file_t* file_handle );
static wiced_result_t filex_shim_file_delete      ( wiced_filesystem_t* fs_handle, const char* filename );
static wiced_result_t filex_shim_dir_open         ( wiced_filesystem_t* fs_handle, wiced_dir_t* dir_handle, const char* dir_name );
static wiced_result_t filex_shim_dir_read         ( wiced_dir_t* dir_handle, char* name_buffer, unsigned int name_buffer_length, wiced_dir_entry_type_t* type, wiced_dir_entry_details_t* details );
static int            filex_shim_dir_end_reached  ( wiced_dir_t* dir_handle );
static wiced_result_t filex_shim_dir_rewind       ( wiced_dir_t* dir_handle );
static wiced_result_t filex_shim_dir_close        ( wiced_dir_t* dir_handle );
static wiced_result_t filex_shim_dir_create       ( wiced_filesystem_t* fs_handle, const char* directory_name );
static wiced_result_t filex_shim_format           ( wiced_block_device_t* device );
...
wiced_filesystem_driver_t wiced_filesystem_driver_filex =
{
    .init             = filex_shim_init            ,
    .mount            = filex_shim_mount           ,
    .unmount          = filex_shim_unmount         ,
    .file_get_details = filex_shim_file_get_details,
    .file_open        = filex_shim_file_open       ,
    .file_seek        = filex_shim_file_seek       ,
    .file_tell        = filex_shim_file_tell       ,
    .file_read        = filex_shim_file_read       ,
    .file_write       = filex_shim_file_write      ,
    .file_flush       = filex_shim_file_flush      ,
    .file_end_reached = filex_shim_file_end_reached,
    .file_close       = filex_shim_file_close      ,
    .file_delete      = filex_shim_file_delete     ,
    .dir_open         = filex_shim_dir_open        ,
    .dir_read         = filex_shim_dir_read        ,
    .dir_end_reached  = filex_shim_dir_end_reached ,
    .dir_rewind       = filex_shim_dir_rewind      ,
    .dir_close        = filex_shim_dir_close       ,
    .dir_create       = filex_shim_dir_create      ,
    .format           = filex_shim_format          ,
};
...
static wiced_result_t filex_shim_format           ( wiced_block_device_t* device )
{
    ...
    FX_MEDIA* media  = &fs_handle.data.filex.handle;
    void*     buffer = PLATFORM_L1_CACHE_PTR_ROUND_UP( (void*) &media[1] ); /* Buffer follows the media structure */
    ...
    fx_media_format_oem_name_set( (const UCHAR*) "Broadcom" );

    number_of_sectors = (ULONG) ( device->device_size / DEFAULT_SECTOR_SIZE );

    /* Format the device */
    filex_result = fx_media_format( media,
                             wiced_filex_driver,               /* Driver entry             */
                             device,                           /* RAM disk memory pointer  */
                             buffer,                           /* Media buffer pointer     */
                             FILEX_MEDIA_BUFFER_SIZE,          /* Media buffer size        */
                             "",                               /* Volume Name              */
                             2,                                /* Number of FATs           */
                             512,                              /* Directory Entries        */
                             0,                                /* Hidden sectors           */
                             number_of_sectors,  /* Total sectors            */
...
{
    UINT filex_result;
    FX_MEDIA* media  = &fs_handle_out->data.filex.handle;
    void*     buffer = PLATFORM_L1_CACHE_PTR_ROUND_UP( (void*) &media[1] ); /* Buffer follows the media structure */

    /* Some devices require formatting before mounting (e.g. RAM disks ) */
    if ( device->init_data->volatile_and_requires_format_when_mounting == WICED_TRUE )
    {
        wiced_result_t result;
        result = filex_shim_format( device );
    ...
    filex_result =  fx_media_open( media, NULL, wiced_filex_driver, device, buffer,  FILEX_MEDIA_BUFFER_SIZE );
    if ( filex_result != FX_SUCCESS )
    {
        return WICED_FILESYSTEM_ERROR;
    }
...
static wiced_result_t filex_shim_unmount          ( wiced_filesystem_t* fs_handle )
{
    ...
    filex_result = fx_media_close( media );
    if ( filex_result != FX_SUCCESS )
    {
        return WICED_FILESYSTEM_ERROR;
    }
...
static wiced_result_t filex_shim_file_close       ( wiced_file_t* file_handle )
{
    ...
    FX_MEDIA* media         = &file_handle->filesystem->data.filex.handle;
    UINT      filex_result;
...
static wiced_result_t filex_shim_dir_read         ( wiced_dir_t* dir_handle, char* name_buffer, unsigned int name_buffer_length, wiced_dir_entry_type_t* type, wiced_dir_entry_details_t* details )
{
    FX_MEDIA* media = &dir_handle->filesystem->data.filex.handle;
    UINT      filex_result;
