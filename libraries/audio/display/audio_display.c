...
#define SONG_DETAILS_DURATION_STRLEN        14   /* Song duration must be less than 1000 minutes */
...
wiced_result_t audio_display_create_management_thread(wiced_thread_t* thread)
{
    ...
    result = wiced_rtos_create_thread(thread, WICED_APPLICATION_PRIORITY, "Audio Display Thread", audio_display_main, DISPLAY_MANAGEMENT_THREAD_SIZE, NULL);
