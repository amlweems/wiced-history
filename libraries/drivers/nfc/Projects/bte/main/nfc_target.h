...
/* MIU for SNEP              */
#ifndef NFA_SNEP_MIU
#define NFA_SNEP_MIU                    (LLCP_MIU <= LLCP_MAX_MIU ? LLCP_MIU : LLCP_MAX_MIU)
#endif

/* Receiving Window for SNEP */
#ifndef NFA_SNEP_RW
#define NFA_SNEP_RW                     5
#endif
...
#if (NFC_BRCM_NOT_OPEN_INCLUDED == TRUE)
    /* For non-open-source, allow the option to use shared NFC/HAL resources */
#undef HAL_WRITE
#ifdef NFC_HAL_SHARED_GKI
    /* HAL is using shared GKI resources as the NFC stack. Do not free msg buf after sending it to HAL */
    #define HAL_WRITE(p)    {nfc_cb.p_hal->write_buf((NFC_HDR *)p);}
#else
    /* HAL is not using shared GKI resources as the NFC stack. Free msg buf after sending it to HAL */
    #define HAL_WRITE(p)    {nfc_cb.p_hal->write(p->len, (UINT8 *)(p+1) + p->offset); GKI_freebuf(p);}
#endif

#ifdef TESTER
    /* Compiling for Insight TestTool (supports run-time selection for calling hal->write or hal->writebuf) */
    #undef HAL_WRITE
    #define HAL_WRITE(p)    {nfc_hal_write((NFC_HDR *)p);}
#endif /* TESTER */
#endif /* NFC_BRCM_NOT_OPEN_INCLUDED */
