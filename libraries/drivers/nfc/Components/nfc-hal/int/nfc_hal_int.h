...
#define NFC_HAL_TASK_EVT_RX_IRQ                 EVENT_MASK (APPL_EVT_1)
...
* BT HCI definitions
*****************************************************************************/
#if defined (EMB_TGT_INCLUDED) && (EMB_TGT_INCLUDED == FALSE)
...
enum
{
    ...
    NFC_HAL_POWER_MODE_CE_LP,           /* NFCC is CE low power mode    */
#endif
...
#if (NFC_BRCM_NOT_OPEN_INCLUDED == TRUE)
/* NFC HAL state in CE low power mode */
enum
{
    NFC_HAL_CE_LP_STATE_TRANSIT,       /* NFCC is going to CE low power mode */
    NFC_HAL_CE_LP_STATE_SLEEP,         /* NFCC is in CE low power mode       */
    NFC_HAL_CE_LP_STATE_WAKE           /* NFCC wakes up in CE low power mode */
};
typedef UINT8 tNFC_HAL_CE_LP_STATE;
#endif
...
#if (NFC_BRCM_NOT_OPEN_INCLUDED == TRUE)
    NFC_HAL_CE_LP_READY_RX_CMD_EVT,         /* NFCC is ready to receive CMD */
    NFC_HAL_CE_LP_LINK_ACTIVATED_EVT,       /* NFC RF link is activated     */
    NFC_HAL_CE_LP_LINK_DEACTIVATED_EVT,     /* NFC RF link is deactivated   */
#endif
    NFC_HAL_LP_LAST_EVT
};
...
#if (NFC_BRCM_NOT_OPEN_INCLUDED == TRUE)
    NFC_HAL_INIT_STATE_W4_AUTOBAUD,        /* Waiting for auto-baud rsp             */
#endif
...
typedef struct
{
    ...
    /* I2C-patch */
    const UINT8         *p_spd_patch;           /* pointer to spd patch             */
...
typedef struct
{
    const UINT8         *p_patch;               /* patch for i2c fix                */
...
#if (NFC_BRCM_NOT_OPEN_INCLUDED == TRUE)
    tNFC_HAL_CE_LP_STATE    power_state;            /* NFCC power state in a power mode         */
#endif
...
enum
{
    HAL_BRCM_API_ENTER_CE_LOW_PWR_EVT,  /* HAL_NfcEnterCELowPowerMode () is called */
    HAL_BRCM_API_ENTER_FULL_PWR_EVT,    /* HAL_NfcEnterFullPowerMode () is called  */
    HAL_BRCM_API_UPDATE_SNOOZE_EVT,     /* HAL_NfcUpdateSnoozeMode () is called    */
    HAL_BRCM_API_UPDATE_BAUD_RATE_EVT   /* HAL_NfcUpdateUpdateBaudRate () is called*/
};

/* data type for HAL_BRCM_API_UPDATE_SNOOZE_EVT */
typedef struct
{
    NFC_HDR                         hdr;
    tHAL_NFC_SNOOZE_CONFIG          snooze_config;
} tNFC_HAL_BRCM_UPDATE_SNOOZE_EVT;

/* data type for HAL_BRCM_API_UPDATE_BAUD_RATE_EVT */
typedef struct
{
    NFC_HDR                         hdr;
    UINT8                           baud_rate_index;
} tNFC_HAL_BRCM_UPDATE_BAUD_RATE_EVT;

/* union of all event data types for BRCM HAL features */
typedef union
{
    NFC_HDR                             hdr;
    tNFC_HAL_BRCM_UPDATE_SNOOZE_EVT     update_snooze;
    tNFC_HAL_BRCM_UPDATE_BAUD_RATE_EVT  update_baud_rate;
} tNFC_HAL_BRCM_EVENT_DATA;

/* nfc_hal_brcm.c */
void nfc_hal_brcm_init (void);
void nfc_hal_brcm_start_auto_baud_detection (void);
void nfc_hal_brcm_reset_baud_on_shutdown (void);
void nfc_hal_brcm_set_local_baud_rate (UINT8 baud);
void nfc_hal_brcm_enter_ce_low_power_mode (void);
void nfc_hal_brcm_enter_full_power_mode (void);
void nfc_hal_brcm_update_snooze_mode (NFC_HDR *p_msg);
void nfc_hal_brcm_update_baud_rate (NFC_HDR *p_msg);
BOOLEAN nfc_hal_brcm_proc_rx_nci_msg (NFC_HDR *p_msg);
BOOLEAN nfc_hal_brcm_power_mode_execute (tNFC_HAL_LP_EVT event);
