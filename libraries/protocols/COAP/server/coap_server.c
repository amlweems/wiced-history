...
static wiced_result_t coap_server_process_request( wiced_coap_server_t* server, wiced_packet_t* packet );
    ...
    while ( 1 )
    {
        ...
        switch ( current_event.event_type )
        {
            case COAP_RECEIVE_EVENT:
                /* Receive Event */
                if ( wiced_udp_receive( &server->socket, &packet, WICED_TIMEOUT ) != WICED_SUCCESS )
                {
                    WPRINT_LIB_ERROR (("Error in receiving UDP packet\n"));
                }
                if ( coap_server_process_request( server, packet ) != WICED_SUCCESS )
                {
...
static wiced_result_t coap_server_process_request( wiced_coap_server_t *server, wiced_packet_t *packet )
{
    ...
    coap_find_service( server, &request_info, &service );
...
static wiced_result_t coap_find_service( wiced_coap_server_t* server, coap_request_info_t* pkt, wiced_coap_server_service_t** service )
{
    ...
    coap_get_service_name( pkt, COAP_OPTION_URI_PATH, buffer );
...
static void coap_get_service_name( const coap_request_info_t* request, uint8_t num, char *data )
{
    size_t i;
