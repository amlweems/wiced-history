...
static wiced_result_t smartbridge_att_cache_discover_all( bt_smartbridge_att_cache_t* cache, uint16_t connection_handle )
{
    ...
    WPRINT_LIB_INFO(("[Cache] All services discovered. count: %u\n",(unsigned int) primary_service_count ));
    ...
    for ( i = 0; i < primary_service_count; i++ )
    {
        ...
        */
        ...
        for ( j = 0; j < characteristic_count; j++ )
        {
            ...
            {
                WPRINT_LIB_INFO(("[Cache] Characteristic Read-Property IS set\n"));
            ...
            {
                WPRINT_LIB_INFO(("[Cache] Characteristic Read-Properties is NOT set\n"));
    ...
    //wiced_bt_smart_attribute_print_list( &(cache->attribute_list) );
...
static wiced_bool_t   smartbridge_att_cache_find_by_device_callback( linked_list_node_t* node_to_compare, void* user_data )
{
    ...
    return ( ( memcmp( cached_attributes->remote_device.address, remote_device->address, sizeof( remote_device->address ) ) == 0 ) && ( cached_attributes->remote_device.address_type == remote_device->address_type) ) ? WICED_TRUE : WICED_FALSE;
}
