...
{
    wiced_crypto_get_random(dest, size);
    return 1;
}
...
int uECC_make_key( uint8_t public_key[ uECC_BYTES * 2 ], uint8_t private_key[ uECC_BYTES ] )
{
    ...
    {
        if ( g_rng_function( (uint8_t *) private, sizeof( private ) ) && EccPoint_compute_public_key( &public, private ) )
        {
...
static uECC_word_t vli2_sub( uECC_word_t *result, const uECC_word_t *left, const uECC_word_t *right )
{
    uECC_word_t borrow = 0;
    wordcount_t i;
